...
| [BRIEF_OPEN_QUESTIONS.md](BRIEF_OPEN_QUESTIONS.md) | Was down to three questions, each needing live Kosli or GitHub rather than readable source: a pull request with two distinct authors, one real root-commit trail, and whether a report survives a real `kosli attest custom`. Round 5 answered everything else it asked — including refuting the claim that control 43 runs the `opa` binary, which it does not; see [INTEGRATION.md](../INTEGRATION.md). *(done, partly — round 6 had no `kosli` binary and no GitHub (proxy 407), so the two live questions stay open. It settled the one that mattered from the collector's source instead: the substitute **cannot** land on a non-root commit, so the port is not too permissive. The report was schema- and jq-validated locally; only the server is untested.)* |
| [BRIEF_1068_CONTRACT.md](BRIEF_1068_CONTRACT.md) | Did control 1068's TypeScript validate the payload it uploads to factstore, or only type it? Plus the four fail-opens and the tests — investigations 3, 6 and 7 of the 1068 brief, which that round did not reach. Needed no repo and no `opa`: bank-side source only. *(done, in full — **no schema and no runtime validation on the way out**; the one guard is on the Jira response. `apidown` is fail-closed, and the one real fail-open is a 404 dropped into neither ticket list. See [INTEGRATION.md](../INTEGRATION.md).)* |
...
`kosli evaluate --policy` takes one file and parses it as a single module that must
be `package policy`, so the library cannot be imported on that path — it has to be
merged, and merging collides with the two rule names the CLI reserves.
`bundle.py` renames the library's API out of the way and then proves the merge
preserved behaviour by diffing both forms against the same input:
